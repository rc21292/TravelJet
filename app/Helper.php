...
class Helper extends Model
{
    ...
    public static function getCategoryImage($image)
    {
        if (!empty($image)) {
            ...
            return 'uploads/categories/1.png';
        }
    }
    ...
    public static function uploadProfileDocuments($temp_path, $image, $file_name = "")
    {
        $json = array();
        if (!empty($image)) {
            $file_original_name = $image->getClientOriginalName();
    ...
    public static function getJobDurationListProfile($key = "")
    {
        $list = array(
            'weekly' => trans('lang.job_duration.weekly'),
            'monthly' => trans('lang.job_duration.monthly'),
