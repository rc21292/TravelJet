...
class SiteManagementController extends Controller
{
    ...
    public function settings()
    {
        ...
        $connects_amount = !empty($settings[0]['connects_amount']) ? $settings[0]['connects_amount'] : null;
        $connects_no_of_credit = !empty($settings[0]['connects_no_of_credit']) ? $settings[0]['connects_no_of_credit'] : null;
        ...
        if (file_exists(resource_path('views/extend/back-end/admin/settings/index.blade.php'))) {
            ...
            return view(
                ...
                compact(
                    ...
                    'sender_tagline', 'sender_url', 'email_logo', 'email_banner',
                    'sender_avatar', 'title', 'email', 'logo', 'commision',
                    'existing_payment_settings', 'connects_per_job', 'connects_amount','connects_no_of_credit' ,'footer_logo',
                    'footer_desc', 'social_list', 'social_unserialize_array',
                    'footer_copyright', 'pages', 'menu_pages', 'menu_pages_1',
                    'unserialize_menu_array', 'menu_title_1', 'menu_title_2', 'menu_title',
                    'client_id', 'payment_password', 'existing_payment_secret',
    ...
    public function getCoinSettings()
    {
    ...
    public function storeCommissionSettings(Request $request)
    {
    	 $json = array();
        if (!empty($request)) {
            $store_commission_settings = SiteManagement::saveCommissionSettings($request);
            if ($store_commission_settings == "success") {
                $json['type'] = 'success';
                return $json;
            } else {
                $json['type'] = 'error';
    ...
    public function storeCoinSettings(Request $request)
    {
        $json = array();
        if (!empty($request)) {
            $store_coin_settings = SiteManagement::saveCoinSettings($request);
            if ($store_coin_settings == "success") {
                $json['type'] = 'success';
                return $json;
            } else {
                $json['type'] = 'error';
