...
const Add = () => {
	...
	useEffect(()=>{
		...
		if (stateqq) {
			...
			setProduct({ ...tutorial, user_id: AppState.user.id });
		}
	...
	const saveProduct = () => {
		var data = {
			...
			from_places: tutorial.from_places,
			stopeges: tutorial.stopeges,
			to_places: tutorial.to_places,
			arrival: tutorial.arrival,
			pickup: tutorial.pickup_time,
			name: tutorial.name,
			in_city: tutorial.in_city,
			no_of_adults:tutorial.no_of_adults,
			no_of_childrens:tutorial.no_of_childrens,
			no_of_infants:tutorial.no_of_infants,
			vehicle_type:tutorial.vehicle_type,
			vehicle_when:tutorial.vehicle_when,
			booking_name:tutorial.booking_name,
			email:tutorial.email,
			mobile:tutorial.mobile,
			description:tutorial.description,
			otp:tutorial.otp,
			vehicle_budget:tutorial.vehicle_budget
		};
		axios({
			method: 'post',
		...
		.then(response => {
			...
			/*setProduct({
				...
				in_city: response.data.in_city,
				stopeges: response.data.stopeges,
				no_of_adults:response.data.no_of_adults,
				no_of_childrens:response.data.no_of_childrens,
				no_of_infants:response.data.no_of_infants,
				vehicle_type:response.data.vehicle_type,
				vehicle_when:response.data.vehicle_when,
				vehicle_budget:response.data.vehicle_budget,
				booking_name:response.data.booking_name,
	...
	const handleChecked = (event) => {
		if (tutorial.in_city == 0) {
			setProduct({ ...tutorial, in_city: 1 });
		}else{
			setProduct({ ...tutorial, in_city: 0 });
		}
	}
   ...
   console.log(e.target);
	...
	const handleAdultClick = event => {
		...
		setProduct({ ...tutorial, no_of_adults: event.currentTarget.dataset.id });
	};

	const handleChildrenClick = event => {
		setClickedChildren(event.currentTarget.dataset.id);
		setProduct({ ...tutorial, no_of_childrens: event.currentTarget.dataset.id });
	};

	const handleInfantsClick = event => {
		setClickedInfant(event.currentTarget.dataset.id);
		setProduct({ ...tutorial, no_of_infants: event.currentTarget.dataset.id });
	};

	const vehicleTypeClick = event => {
		setProduct({ ...tutorial, vehicle_type: event.target.value });
	};

	const vehicleBudgetClick = event => {
		setProduct({ ...tutorial, vehicle_budget: event.target.value });
	};

	const vehicleWhenClick = event => {
		setProduct({ ...tutorial, vehicle_when: event.target.value });
	};
	...
	const handleInputChange = (index, event) => {
		...
		setProduct({ ...tutorial, stopeges: values });
	};
